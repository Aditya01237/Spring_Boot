...
const Login = () => {
  ...
  useEffect(() => {
    // Check for the 'token' parameter in the URL (from Google OAuth redirect)
    ...
    if (errorParam) {
      ...
      // Clear the error parameter from URL without triggering navigation
  ...
  const handleEmailLogin = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || err.response?.data?.message || 'Invalid email or password. Please try again.';
